...
public class HardwareController {
    ...
    public ResponseEntity<String> saveAsset(@Valid @RequestBody SaveHardwareRequestDto saveHardwareRequestDto, HttpServletRequest request) {
        // 세션 가져오기
        ...
        hardwareService.saveHardware(saveHardwareRequestDto);
    ...
    public ResponseEntity<String> updateAsset(@RequestParam Long hwidx, @RequestBody UpdateHardwareRequestDto updateHardwareRequestDto, HttpServletRequest request) {
        ...
        hardwareService.updateHardware(updateHardwareRequestDto, hwidx);
        return new ResponseEntity<>("해당 자산의 수정이 완료되었습니다.", HttpStatus.OK);
    }
    @PostMapping("/assets/hardware/assign")
    public ResponseEntity<String> assignAsset(@RequestParam Long hwidx, @RequestBody AssignHardwareRequestDto assignHardwareDtoRequest, HttpServletRequest request) {
        HttpSession session = request.getSession(false); // false로 설정하면 세션이 없으면 null을 반환
        // 세션이 null이면 에러 반환
        if (session == null) {
            throw new AuthenticationFailedException("해당 작업을 수행하기 위한 권한이 없습니다. 관리자 계정으로 로그인해주세요.");
